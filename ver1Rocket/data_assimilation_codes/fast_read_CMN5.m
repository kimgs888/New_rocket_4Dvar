...
function [info] = fast_read_CMN5(year,DOY,Time,d_Time, Elevation,latstart,latend,lonstart,lonend,numbstatn,satheight,min_dist_stn,Data_location)
                                ...
                                if inuse==1 || numbstatni ==1 
                                   ...
                                   clear data data_array
                                        ...
                                        Time1index = ((roundn(ndtime1,-5) <= Time1) & (Time1 <= roundn(ndtime2,-5))...
                                                    &(abs(Elmat) ~= 90)&(abs(Elmat) > Elevation) & (STEC > 0)& (PRN==15));
                                        ...
                                        if (~isempty(PRN(Time1index))&& ~isempty(Time1(Time1index))&&  ~isempty(STEC(Time1index))...
                                                ...
                                                if ~isempty(sat.lat(ind))
                                                    %check to see if index is empty if so no rays in that file 
