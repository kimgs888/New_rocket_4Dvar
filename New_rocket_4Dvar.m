    ...
    working_dir = '/root/Desktop/Gwangsu/ROCKET1';
    cd (working_dir)
    addpath( working_dir)
    ...
    Final_solution_flder = '/root/Desktop/Gwangsu/ROCKET1/Final_solution'; 
    ...
    Temp_fld_Loc = '/root/Desktop/Gwangsu/ROCKET1'; 
    ...
    % covergence  parameters 
     ...
     B_dump_factor  = 1e-10;%/10; % .5;%(1e-1);% Dump factor on B 
     B_dump_min=1e-23;
    ...
    EbH20 =1e24;            % H2O Emissions value used in estimating variance 
    EbH2  =1e24;            % H2 Emissions value used in estimating variance 
    
    QH20  =(EbH20)^2;       % Variance estimate of the H2O Emisions 
    QH2  = (EbH2)^2;        % Variance estimate of the H2  Emisions 
    
    EoH20 =3e27;            % Intial background estimated of Hydrogen (H2O) 
    EoH2 = 1e27;            % Intial background estimated of Hydrogen (H2) 
    dif=[];
...
%                       end 
    ...
    %                 disp (['For epoch: ', num2str(Year), '_',num2str(DOY),'_',Current_tm_strhrs,'h',Current_tm_strmins,'min'])
                  ...
                  % dif=[factorEb*QH20*sum_EB_adjnt_X_lambdaH20;dif];
                    ...
                    num_iter = iterations
                     ...
                     if  iterations > numb %|| Totval > limtval %(Eb_dif(end,1)-Eb_dif(end-1,1))<0  && length(chi_set)> 5 || ((abs(diffChi) <=100)&& length(chi_set)> 5)||...
                             %((diffChi<100)&& length(chi_set)> 5)
                                figure
