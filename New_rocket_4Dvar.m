    ...
    working_dir = 'C:/ROCKET';
    cd (working_dir)
    addpath( working_dir)
    % Define the Location of Data to be assimilated 
    Data_STEC_CMN_folder = 'C:/ROCKET/Data';

    % Define where the final solution should be saved
    Final_solution_flder = 'C:/ROCKET/Final_solution'; 
    ...
    % covergence  parameters 
     ...
     B_dump_min=1e-23;
     max_while_iter = 200;    % maximum number of iteration that should be performed while 
...
for DOY =DOYS
   ...
   [Data_location]= fomulate_data_location_flder (Data_STEC_CMN_folder,slashx,Year,DOY);
    ...
    % Covert the available data into binary format This data format speeds
    % up the file reading procedure
...
%                       end 
    ...
    %                 disp (['For epoch: ', num2str(Year), '_',num2str(DOY),'_',Current_tm_strhrs,'h',Current_tm_strmins,'min'])
                    ...
                    % current estimate
                    % The lower chi sqaured value is the better the solution 
                    chivaluenew = sum((((H*Xo-STEC)).^2)./(diag(sd)))/length(STEC);
                    ...
                    % Plot the Result
                     if (Eb_dif(end,1)-Eb_dif(end-1,1))<0  && length(chi_set)> 5 || ((abs(diffChi) <=chitol)&& length(chi_set)> 5)
