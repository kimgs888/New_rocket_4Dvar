    ...
    % covergence  parameters 
     ...
     B_dump_factor  = 60;%/10; % .5;%(1e-1);% Dump factor on B 
     B_dump_min=1e-23;
...
%                       end 
                 ...
                 [RMSEK1] = fnxRmse1(XK,H,STEC,diffusiontimex,iterations, RMSEK1,RxLat,RxLon,PRN);
                % *****************Quality control *********************************************%
    ...
    %                 disp (['For epoch: ', num2str(Year), '_',num2str(DOY),'_',Current_tm_strhrs,'h',Current_tm_strmins,'min'])
                    ...
                    num_iter = iterations
                     ...
                     if (Eb_dif(end,1)-Eb_dif(end-1,1))<0  && length(chi_set)> min_iter || ((abs(diffChi) <=1e-6)&& length(chi_set)> min_iter)||...
                             ((diffChi<1e-6)&& length(chi_set)> min_iter)
                                figure
